...
//Pojet Mbed : Un jeux de socité ou deux joueurs s'affronte jusqu'à ce que l'un d'eux gagne 3 mini-jeux. On voit leur avancement 
//Entre chaque minijeux. Les mini jeux disponible sont un pierre feuilles ciseau, un devine le nombre et un jeux de mémoir avec les leds.

// Initialisation les entréé et sortie sur le mBed
//L'écran
C12832 LCD(p5,p7,p6,p8,p11);
// Les position du joystick
DigitalIn ju(p15);
DigitalIn jd(p12);
DigitalIn jl(p13);
DigitalIn jc(p14);
DigitalIn jr(p16);
//Les deux potentiomètre
AnalogIn pot1 (p19);
AnalogIn pot2 (p20);
//Les LEDS
...
///Fonction contenant les jeu///
/* Devine le nombre, le jeux consiste à deviner un nombre choisi au hazard
@param :  Le nombre de joueur
...
int GuessNum(int Players){
    ...
    t=0;
    while(s1!= 3 and s2!= 3){  //On sort de ce while si l'un des deux jouers atteint 3 points
        t=0; //t gets reset
        res=rand()%100; //génère le nombre a deviner entre 0 et 1 random
        res=res/100;
        while(t<5){ //On sort de cette boucle quand le timer s'écroule/quand on dépase le temps donné
            g1=pot1.read(); //L'entré du joueur 1
            if(Players==1){
                g2=rand()%100;  //L'entré de l'ordinateur
                g2=g2/100;
            }
...
/* Jeux de pierre feuille ciseau. Les joueurs utilise le joysticks pour indiquer leur objet. Celui qui gagne 3 points gagne la manche.
@param :  Le nombre de joueur
return : le joueur qui à gagné
*/
int PFC(int Players){
 int res;
    ...
    while(s1!= 3 and s2!= 3){  //On sort de ce while si l'un des deux joueurs atteint 3 points
	    g1= 4;
        g2= 4;
	    LCD.cls();
        LCD.locate(0,0);
...
/* Jeux de mémoire, des LEDs s'allume successivement et les joueur doivent chacun leur tour executé le même paterne à l'aide du joysticks.
@param :  Le nombre de joueur
return : le joueur qui à gagné
*/
...
int main() {
    ...
    int PlayersN=0;
    LCD.locate(0,0);
    LCD.printf("Selectionez le nombre de joueur:\nJoystick haut pour 1\nJoystick bas pour 2");
    ...
    while(r==0){
        ...
        if(G2>112 and G1>112){
            LCD.cls();
            LCD.printf("Les deux joueurs gagnent !");
            r=3;
        }
        wait(0.2);
    }
